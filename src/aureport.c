...
// New addition of header files
#include "hashtab-util.h"
#include "hashtab.h" 
#include "queue.h"

// New addition: other defs
...
static lol lo;
static htable *ev_table; // New addition
static int found = 0;
...
int main(int argc, char *argv[])
{
	...
	* with restrictions. Not a fatal error.  */
	...
	// New addition: create the hash table and ready event queue
	/** re_queue is external in ausearch-lol.c **/
	re_queue = queue_create();
	ev_table = ht_create(TABLE_SIZE, compar, hash);
	lol_create(&lo);
	if (user_file)
		...
		rc = process_logs(&config);
	// New addition: destroy the hash table and queue
	...
	*l = get_ready_event(&lo);
	...
	while (1) {
		...
		rc = fgets_unlocked(buff, MAX_AUDIT_MESSAGE_LENGTH,
					log_fd);
		if (rc) {
		  // New addition: added hashtab as arg
		        if (lol_add_record(&lo, ev_table, buff)) {
				*l = get_ready_event(&lo);
				if (*l)
					break;
			}
			...
			if (feof_unlocked(log_fd)) {
			  ...
			  terminate_all_events(&lo, ev_table);
				*l = get_ready_event(&lo);
				if (*l)
					return 0;
				else
