...
// New addition: made lol_append return the index where it appended l,
...
/**
 * Check to see if (sec2, milli2) is at least event_window
 * milliseconds apart.
 *
 * Returns 1 if this is the case,
 *         0 otherwise.
 **/
static int check_times(time_t sec1, unsigned int milli1, time_t sec2, unsigned int milli2) {
	if (((int)sec2 * 1000 + milli2) - ((int)sec1 * 1000 + milli1) > event_window)
		return 1;
...
// New addition: htable is arg and milliseconds
static void check_events(lol *lo, htable *table, time_t sec, unsigned int milli)
{
	int i;
	...
	for(i=0;i<=lo->maxi; i++) {
		...
		if (cur->status == L_BUILDING) {
			if (check_times(cur->l->e.sec, cur->l->e.milli, sec, milli)) {
			// If 2 seconds have elapsed, we are done
			
		        //if (cur->l->e.sec + 2 < sec) { 
				cur->status = L_COMPLETE;
				
				queue_push(re_queue, &i);

				// Remove this event from the hash table
				char *event_key = (char *)malloc(20 * sizeof(char));
				event_to_key(event_key, &cur->l->e);
				ht_entry *removed = ht_remove(table, event_key, &i);
				if (! removed)
				  fprintf(stderr, "COULD NOT REMOVE EVENT!!");
				free(removed->datum);
				free(removed);
				free(event_key);
...
int lol_add_record(lol *lo, htable *table, char *buff)
{
	...
	/* Find if the new record has any other records
	 * that match its timestamp, so we can look up the corresponding event
	 * in the lol. The hashtable here is storing EVENTS.
	 */
	char *record_key = (char *)malloc(20 * sizeof(char));
	if (! record_key) {
	  fprintf(stderr, "Could not allocate space to insert into the hash table.");
	  return 0;
	}

	snprintf(record_key, 20, "%d%d", e.sec, e.milli);

	ht_entry *matches = ht_find(table, record_key);
	
	if (matches) {
	  ht_entry *cur = matches;

	  while (NULL != cur) {
	    int bucket = 0;
	    memcpy(&bucket, cur->datum, sizeof(int));

	    if (lo->array[bucket].status != L_BUILDING) continue;
	...
	newentry->datum = malloc(8); //sizeof(char));
	strncpy(newentry->key, record_key, 256);

	// Free the search key
	free(record_key);

	// The new datum is the bucket in the lol where we put the new event.
	int where = lol_append(lo, l);
	memcpy(newentry->datum, &where, sizeof newentry->datum);
	ht_insert(table, newentry);

	// New addition: ht is arg., added milliseconds also
	check_events(lo, table, e.sec, e.milli);
	return 1;
...
void terminate_all_events(lol *lo, htable *table)
{
	...
	for (i=0; i<=lo->maxi; i++) {
		...
		if (cur->status == L_BUILDING) {
			...
			// New addition (S)
			// (Add event to ready event queue)
			queue_push(re_queue, &i);

			// fprintf(stdout, "TERMINATE ALL EVENTS: BING!\n");
...
llist* get_ready_event(lol *lo)
{
	...
	// fprintf(stderr, "READY IS %d\n", ready);

	ready--;
	// fprintf(stderr, "Flushing a ready event!!\n");
	// fprintf(stderr, "The queue has %d items in it.\n", re_queue->size);
	int *lol_bucket;
	lol_bucket = (int *)queue_pop(re_queue);
	lolnode *node = &lo->array[*lol_bucket];
	free(lol_bucket);
	// fprintf(stderr, "GET_READY_EVENT: lolnode has a list of %d items.\n", node->l->cnt);
